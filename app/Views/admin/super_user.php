            ...
            <tbody>
                ...
                <tr >
                    ...
                    <td style="vertical-align: top;" class="text-center">
                      ...
                      <?php }else{ ?>
					             <input type="checkbox" data-id="<?=$v['user_id'];?>" name="my-checkbox" id="switch<?=$i;?>"  data-bootstrap-switch data-off-color="danger" checked="true" data-on-color="success" data-on-text="ON" data-off-text="OFF" <?php if($v['user_id']==1){?> disabled <?php } ?>>
                      <?php } ?>
					          </td>
                    <td class="text-center" style="vertical-align: top;">
							       <a href="<?=base_url('index.php/admin/getprofile_super/'.$v['user_id'])?>" class="btn btn-success btn-sm"><i class="fas fa-search-plus"></i>
							       </a>
        							<?php if($v['user_id'] == session()->get('user_id') or session()->get('checktype')=='admin'){?>
        							<a href="<?=base_url('index.php/admin/editprofile_super/'.$v['user_id'])?>" class="btn btn-primary btn-sm"><i class="fas fa-edit"></i>
        							</a>
...
//$(document).ready(function() { 
  ...
  $('#example1').on('switchChange.bootstrapSwitch', 'input[name="my-checkbox"]', function(event, state) {
		...
		//if ($(this).bootstrapSwitch('state')) {     
			...
			$.ajax({
				method: 'POST',
				url: "<?=base_url('index.php/admin/upstatus_super')?>",
          ...
          <?php session()->setFlashdata('success', '<i class="fe fe-check-circle fs-16"></i> แก้ไขข้อมูลสำเร็จแล้ว');?>
					location.href = "<?=base_url('index.php/admin/super_user')?>";
				}
			});
		//}else {
         //   alert("sssssssssssss");
...
function deleteUser(UserID){
  ...
  .then((delmysuper_user) => {
    if(delmysuper_user){
      location.href = "<?=base_url();?>/admin/delmysuper_user/" + UserID;
    }
  });
}
