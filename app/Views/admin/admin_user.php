    ...
    <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
            ...
            <tbody>
                ...
                <tr >
                    ...
                    <td class="text-center" style="vertical-align: top;">
							       <a href="<?=base_url('index.php/admin/getprofile_admin/'.$v['user_id'])?>" class="btn btn-success btn-sm"><i class="fas fa-search-plus"></i>
							       </a>
        							<?php if($v['user_id'] == session()->get('user_id') or session()->get('checktype')=='admin'){?>
        							<a href="<?=base_url('index.php/admin/editprofile_admin/'.$v['user_id'])?>" class="btn btn-primary btn-sm"><i class="fas fa-edit"></i>
        							</a>
...
//$(document).ready(function() { 
  ...
  $('#example1').on('switchChange.bootstrapSwitch', 'input[name="my-checkbox"]', function(event, state) {
		...
		//if ($(this).bootstrapSwitch('state')) {     
			...
			$.ajax({
				method: 'POST',
				url: "<?=base_url('index.php/admin/upstatus_admin')?>",
          ...
          <?php session()->setFlashdata('success', '<i class="fe fe-check-circle fs-16"></i> แก้ไขข้อมูลสำเร็จแล้ว');?>
					location.href = "<?=base_url('index.php/admin/admin_user')?>";
				}
			});
		//}else {
         //   alert("sssssssssssss");
...
function deleteUser(UserID){
  ...
  .then((delmyadmin_user) => {
    if(delmyadmin_user){
      location.href = "<?=base_url();?>/admin/delmyadmin_user/" + UserID;
    }
  });
}
