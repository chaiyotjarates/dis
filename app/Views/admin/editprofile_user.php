    ...
    <section class="content">
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <form action="<?=base_url('index.php/admin/updateprofile_user')?>" name="profile" id="profile" method="post" autocomplete="off"  enctype="multipart/form-data">
            ...
            <div class="form-group row">
              <label for="user_clus" class="col-sm-3 col-form-label">เขตตรวจราชการ <cite class="req-text">*</cite></label>
              <div class="col-sm-9">
                <select name="user_clus" id="user_clus" class="form-control" style="max-width:250px;" required>
                  ...
                  <option value="<?=$a['clus_id']?>" <?php if($a['clus_id'] == $user['user_clus']): echo 'selected'; endif; ?>><?=$a['clus_name']?></option>
                  <?php } ?>
                </select>
              </div>
            </div>
            ...
            <div class="form-group row">
              <label for="user_area" class="col-sm-3 col-form-label">เขตพื้นที่การศึกษา <cite class="req-text">*</cite></label>
              <div class="col-sm-9">
                <select name="user_area" id="user_area" class="form-control" style="max-width:250px;" required>
                  ...
                  <option value="<?=$v['area_code']?>" <?php if($v['area_code'] == $user['user_area']): echo 'selected'; endif; ?>><?=$v['area_name']?></option>
                  <?php } ?>
                </select>
              </div>
            </div>
            ...
            <div class="form-group row">
              <label for="user_area" class="col-sm-3 col-form-label">โรงเรียน <cite class="req-text">*</cite></label>
              <div class="col-sm-9">
                <select name="user_sch" id="user_sch" class="form-control" style="max-width:250px;" required>
                  <option value="">-------เลือกโรงเรียน------</option>
                  <?php foreach($school as $v){ ?>
                  <option value="<?=$v['sch_id']?>" <?php if($v['sch_id'] == $user['sch_id']): echo 'selected'; endif; ?>><?=$v['sch_name']?></option>
                  <?php } ?>
                </select>
              </div>
            </div>
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-9">
                <select name="user_role" id="user_role" class="form-control" style="max-width:250px;" required>
                  ...
                  <option value="Admin" <?php if($user['user_role']=="Admin"){ echo 'selected'; }?>>ผู้ดูแลระบบ</option>
                  <option value="User" <?php if($user['user_role']=="User"){ echo 'selected'; }?>>สมาชิก</option>
                </select>
              </div>
            </div>
...
$(document).on('click', '#btn-submit', function(e) {
    ...
    .then((updateprofile_user) => {
      if(updateprofile_user){
        $('#profile').submit();
      }
    });
});
...
$(document).ready(function(){
        ...
        $('#user_clus').change(function() {
            ...
            success: function(data) {
                ...
                $('#user_area').html(html);
                $('#user_sch').html('<option value="">เลือกโรงเรียน</option>');
            }
            });
            return false;
        });
        ...
        $('#user_area').change(function() {
            ...
            success: function(data) {
                ...
                for (i = 0; i < data.length; i++) {
                html += '<option value=' + data[i].sch_id + '>' + data[i].sch_name + '</option>';
                }
                $('#user_sch').html(html);
            }
            });
