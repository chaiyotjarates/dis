...
<div class="card">
    ...
    <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
            ...
            <tbody>
                ...
                <tr >
                    ...
                    <td style="vertical-align: top;">
					<?=ShortDateThai($v['co_date_start']);?>
                    </td>
                    <td style="vertical-align: top;">
					<?=ShortDateThai($v['co_date_end']);?>
                    </td>
                    <td style="vertical-align: top;" class="text-center">
                      <?php if($v['co_status']=="ไม่อนุมัติ" || $v['co_status']=="รอดำเนินการ"){?>
					  <input type="checkbox" name="my-checkbox" data-id="<?=$v['co_id'];?>" id="switch<?=$i;?>" data-bootstrap-switch data-off-color="success" checked="true" data-on-color="danger" data-on-text="OFF" data-off-text="ON">
                      <?php }else{ ?>
					  <input type="checkbox" data-id="<?=$v['co_id'];?>" name="my-checkbox" id="switch<?=$i;?>"  data-bootstrap-switch data-off-color="danger" checked="true" data-on-color="success" data-on-text="ON" data-off-text="OFF">
                      <?php } ?>
					 </td>
                    <td class="text-center" style="vertical-align: top;">
						<a href="<?=base_url('index.php/admin/edit_config/'.$v['co_id'])?>" class="btn btn-primary btn-sm"><i class="fas fa-edit"></i></a>
						<a onclick="deleteConfig('<?=$v['co_id'];?>')" class="btn btn-danger btn-sm" href="#"><i class="fa fa-trash"></i></a>
                    </td>
                </tr>
                <?php $i++;} ?>
            </tbody>
  ...
  <?php echo view('template/script'); ?>
    ...
    //$(document).ready(function() { 
        ...
        $('#example1').on('switchChange.bootstrapSwitch', 'input[name="my-checkbox"]', function(event, state) {
    		...
    		//if ($(this).bootstrapSwitch('state')) {     
    			...
    			$.ajax({
    				method: 'POST',
    				url: "<?=base_url('index.php/admin/upstatus_config')?>",
    				data: {'my_checkbox_value': ConID},
    				//dataType: 'json',
    				//cache: false,
    				success: function(data){
    					...
    					location.href = "<?=base_url('index.php/admin/list_config')?>";
    				}
    			});
    		//}else {
             //   alert("sssssssssssss");
    ...
    function deleteConfig(ConID){
      ...
      .then((del_config) => {
        if(del_config){
          location.href = "<?=base_url();?>/index.php/admin/del_config/" + ConID;
        }
      });
    }
