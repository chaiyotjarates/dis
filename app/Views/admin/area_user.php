...
foreach($cluster as $c){ 
  $Clus_ID[$c['clus_id']] = $c['clus_id'];
  $Clus_Name[$c['clus_id']] = $c['clus_name'];
  $Clus_Short[$c['clus_id']] = $c['clus_short'];
}
// level_order
$Area_Code = array();
$Area_Name = array();
foreach($area as $a){ 
  $Area_Code[$a['area_code']] = $a['area_code'];
  $Area_Name[$a['area_code']] = $a['area_name'];
}
?>    
<!-- Default box -->
<div class="card">
    ...
    <table id="example1" class="table table-bordered table-striped responsive" style="width:100%">
            ...
            <tbody>
                ...
                <tr >
                    ...
                    <td class="text-center" style="vertical-align: top;">
							       <a href="<?=base_url('index.php/admin/getprofile_area/'.$v['user_id'])?>" class="btn btn-success btn-sm"><i class="fas fa-search-plus"></i>
							       </a>
        							<?php if($v['user_id'] == session()->get('user_id') or session()->get('checktype')=='admin'){?>
        							<a href="<?=base_url('index.php/admin/editprofile_area/'.$v['user_id'])?>" class="btn btn-primary btn-sm"><i class="fas fa-edit"></i>
        							</a>
...
//$(document).ready(function() { 
  ...
  $('#example1').on('switchChange.bootstrapSwitch', 'input[name="my-checkbox"]', function(event, state) {
		...
		//if ($(this).bootstrapSwitch('state')) {     
			...
			$.ajax({
				method: 'POST',
				url: "<?=base_url('index.php/admin/upstatus_area')?>",
          ...
          <?php session()->setFlashdata('success', '<i class="fe fe-check-circle fs-16"></i> แก้ไขข้อมูลสำเร็จแล้ว');?>
					location.href = "<?=base_url('index.php/admin/area_user')?>";
				}
			});
		//}else {
         //   alert("sssssssssssss");
...
function deleteUser(UserID){
  ...
  .then((delmyarea_user) => {
    if(delmyarea_user){
      location.href = "<?=base_url();?>/admin/delmyarea_user/" + UserID;
    }
  });
}
